...
var debug = require('debug')('amqp:connect');
...
function openOptionsFromURL(parts, extraClientProperties) {
  var user = 'guest',
    passwd = 'guest';
  if (parts.auth) {
...
function connect(url, socketOptions, openCallback) {
  debug('.connect', url, socketOptions);
  ...
  var parts = urlLib.parse(url, true); // yes, parse the query string
  var protocol = parts.protocol;
  var noDelay = !!sockopts.noDelay;

  var extraClientProperties = sockopts.clientProperties || {};
  var options = openOptionsFromURL(parts, extraClientProperties);
  ...
  function onConnect() {
    ...
    c.open(options, function (err, ok) {
      if (err) {
        debug('.connect.onConnect Connection.open err');
        openCallback(err);
      } else {
        debug('.connect.onConnect Connection.open no err');
        openCallback(null, c);
      }
    });
  }
  ...
  sock.once('error', function (err) {
    debug('connect.sock.error');
    if (!sockok) {
      debug('connect.sock.error sockok = false');
      openCallback(err);
    }
  });
