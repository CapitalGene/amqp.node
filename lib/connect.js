...
function connect(url, socketOptions, openCallback) {
  ...
  if (typeof url === 'function') {
    ...
    url = null;
    socketOptions = null;
  } else if (typeof socketOptions === 'function') {
    openCallback = socketOptions;
    socketOptions = null;
  }
  ...
  return new Promise(function (resolve, reject) {
      function onConnect() {
        ...
        c.open(fields, function (err, ok) {
          ...
          if (err) {
            debug('.connect.onConlnect Connection.open err', err);
            return reject(err);
          } else {
      ...
      if (timeout) {
        sock.setTimeout(
          timeout,
          function () {
            return reject(new Error('connect ETIMEDOUT'));
          });
      }

      sock.once('error', function (err) {
        if (!sockok) {
          debug('connect.sock.error', '!sockok', err);
          return reject(err);
        }
      });
    })
    .nodeify(openCallback);
}
