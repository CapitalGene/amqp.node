...
var debug = require('debug')('amqp:connection');
...
function mainAccept(maFrame) {
  var rec = this.channels[maFrame.channel];
  if (rec) {
    return rec.channel.accept(maFrame);
  } else {
    // NB CHANNEL_ERROR may not be right, but I don't know what is ..
    this.closeWithError(
      fmt('Frame on unknown channel %d', maFrame.channel),
      constants.CHANNEL_ERROR,
      new Error(fmt('Frame on unknown channel: %s',
        inspect(maFrame, false)))
    );
  }
}
...
Connection.prototype.sendProtocolHeader = function() {
...
*/
Connection.prototype.open = function(allFields, openCallback0) {
  debug('open');
  var self = this;
  ...
  function await(k) {
    return self.step()
      .then(function(sFrame) {
        if (sFrame.channel !== 0) {
          return Promise.reject(
            new Error(
              fmt('Frame on channel != 0 during handshake: %s',
                inspect(sFrame, false)))
          );
        } else {
          return Promise.resolve(k(sFrame));
        }
      })
      .catch(bail);
  }
  ...
  function expect(Method, k) {
    return await(function(eFrame) {
      if (eFrame.id === Method) {
        k(eFrame);
      } else {
        bail(new Error(
          fmt('Expected %s; got %s',
            methodName(Method), inspect(eFrame, false))));
      }
    });
  }
  ...
  function onOpenOk(openOk) {
    ...
    self.accept = mainAccept.bind(self);
    succeed(openOk);
  }
...
Connection.prototype.close = function(closeCallback) {
  debug('close');
...
Connection.prototype.closeBecause = function(reason, code, k) {
...
Connection.prototype.closeWithError = function(reason, code, error) {
  this.emit('error', error);
  this.closeBecause(reason, code);
};

Connection.prototype.onSocketError = function(err) {
...
Connection.prototype.toClosing = function(capturedStack, k) {
  var send = this.sendMethod.bind(this);

  this.accept = function(f) {
    if (f.id === defs.ConnectionCloseOk) {
      debug('toClosed ConnectionCloseOk');
      if (k) {
        k();
      }
...
Connection.prototype._closeChannels = function(capturedStack) {
...
Connection.prototype.toClosed = function(capturedStack) {
  ...
  this.close = function(cb) {
    if (cb) {
      cb(new IllegalOperationError('Connection closed', capturedStack));
    }
  };
  if (this.heartbeater) {
...
Connection.prototype.startHeartbeater = function() {
  if (this.heartbeat === 0) {
    return null;
  } else {
    var self = this;
...
Connection.prototype.freshChannel = function(channel, options) {
...
Connection.prototype.acceptLoop = function() {
  debug('acceptLoop');
  var self = this;
...
Connection.prototype.step = function(cb) {
  debug('step');
  var self = this;
...
Connection.prototype.checkSend = function() {
  var check = this.sentSinceLastCheck;
  this.sentSinceLastCheck = false;
  return check;
};

Connection.prototype.checkRecv = function() {
  var check = this.recvSinceLastCheck;
  this.recvSinceLastCheck = false;
  return check;
};

Connection.prototype.sendBytes = function(bytes) {
...
Connection.prototype.sendMethod = function(channel, Method, fields) {
  var eFrame = encodeMethod(Method, channel, fields);
  this.sentSinceLastCheck = true;
  var buffer = this.channels[channel].buffer;
  return buffer.write(eFrame);
};

Connection.prototype.sendMessage = function(channel,
  Method, fields,
  Properties, props,
  content) {
...
Connection.prototype.sendContent = function(channel, body) {
...
Connection.prototype.recvFrame = function() {
  // debug('recvFrame');
  // %%% identifying invariants might help here?
  var pFrame = parseFrame(this.rest, this.frameMax);
  // debug('recvFrame', 'frame', frame);
  if (!pFrame) {
    // debug('recvFrame', '!pFrame');
    var incoming = this.stream.read();
    if (incoming === null) {
      // debug('recvFrame', '!pFrame', 'incoming === null');
      return false;
    } else {
    ...
    // debug('recvFrame', '!!pFrame');
    this.rest = pFrame.rest;
    return decodeFrame(pFrame);
  }
};
